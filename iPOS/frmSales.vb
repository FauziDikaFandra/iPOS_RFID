...
Public Class frmSales
    ...
    Private Sub Update_MySTAR()
        ...
        If RsMem.Tables(0).Rows.Count > 0 Then
            Call MySTAR(Trim(RsMem.Tables(0).Rows(0).Item("card_number")))
    ...
    End Sub
    Private Sub CmdNav_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles CmdNav.Click
        Dim Index As Short = CmdNav.GetIndex(eventSender)
        Select Case Index
            Case 0 'Enter
    ...
    End Sub
    Private Sub frmSales_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        Frame3.Text = "CUSTOMER - " & VBonus_Point
        'If FromRFID = True Then
        '    ViewRelease(vno_transRFID)
        'End If
        'FromRFID = False
    End Sub
    Private Sub frmSales_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        Call Kosong()
        RFIDStatus = False
    ...
    Private Function Gen_No() As String
        ...
        RsCari = Nothing
    End Function
    Private Sub cmdsales_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdsales.Click
        Dim Index As Short = cmdsales.GetIndex(eventSender)
        Dim aa As String
        Dim bb As String
        GotoPayment = False
        Select Case Index
            Case 0 'Payment
                ...
                VDiscBySTAR = 0
                If cmdsales(9).Enabled Then cmdsales_Click(cmdsales.Item(9), New System.EventArgs())
                'no1
                ...
                frmPayment.vpay.Text = CDbl(vgtotal.Text) - VDiscBySTAR
                frmPayment.txtcard_no.Text = txtcard_no.Text
                frmPayment.vstatus.Text = Me.Text
            ...
            Case 1 'Hold
                If dsSales.Tables(0).Rows.Count = 0 Then Exit Sub
                Call Simpan_Header()
                Call CetakPesan("HOLD", vno_trans.Text)
                Call SaveLog("Hold Transaction " & vno_trans.Text & " " & VKasir_ID & " / " & VKasir_Nm)
                VNomor = ""
                Me.Close()
                frmMain.Show()
            Case 2 'Discount
                If CDbl(vdisc2.Text) = 0 Then
                    frmDisc.lblmsg.Text = "DISCOUNT"
                    frmDisc.Text = "DISCOUNT"
                    frmDisc.ShowDialog()
                Else
                    MsgBox("Key Discount Maksimum 2 X", MsgBoxStyle.OkOnly + MsgBoxStyle.Information, "Oops..")
                End If
                txtkode.Focus()
            Case 3 'Line Void
                If DataGridView1.RowCount > 0 Then
                    If CDbl(DataGridView1.Item(11, DataGridView1.CurrentRow.Index).Value) = 1 Or CDbl(DataGridView1.Item(5, DataGridView1.CurrentRow.Index).Value) < 0 Then
                        MsgBox("Item tersebut tidak dapat divoid", MsgBoxStyle.OkOnly + MsgBoxStyle.Information, "Oops..")
                        Exit Sub
                    End If
                    ...
                    With DataGridView1
                        getSqldb("INSERT INTO Sales_Transaction_Details(Transaction_Number, Seq, PLU, " &
                                 ...
                                 "'" & .Item(2, DataGridView1.CurrentRow.Index).Value & "','" & .Item(3, DataGridView1.CurrentRow.Index).Value & "'," &
                                 "'" & .Item(4, DataGridView1.CurrentRow.Index).Value & "','" & -1 * CDbl(.Item(5, DataGridView1.CurrentRow.Index).Value) & "' " &
                                 ",'" & -1 * CDbl(.Item(4, DataGridView1.CurrentRow.Index).Value) * CDbl(.Item(5, DataGridView1.CurrentRow.Index).Value) & "'," &
                                 "'" & .Item(6, DataGridView1.CurrentRow.Index).Value & "','" & -1 * CDbl(.Item(7, DataGridView1.CurrentRow.Index).Value) & "'," &
                                 "'" & .Item(8, DataGridView1.CurrentRow.Index).Value & "','" & -1 * CDbl(.Item(9, DataGridView1.CurrentRow.Index).Value) & "'," &
                                 "'" & -1 * CDbl(.Item(10, DataGridView1.CurrentRow.Index).Value) & "','0','0','0','" & .Item(1, DataGridView1.CurrentRow.Index).Value & "')", ConnLocal)
                        Call SaveLog(Me.Name & " " & "Simpan_Detail" & vno_trans.Text & " " & .Item(2, DataGridView1.CurrentRow.Index).Value.Trim & " " & Format(Now, "HH:mm:ss"))
                        getSqldb("update Sales_Transaction_Details set flag_void=1 where transaction_number='" & vno_trans.Text & "' and seq='" & .Item(0, DataGridView1.CurrentRow.Index).Value & "'", ConnLocal)
                        'Call SaveLog("Void Transaction " & vno_trans.Text & " " & VSuper_Nm)
                        Call SaveLog(Me.Name & " " & "Void_Detail" & vno_trans.Text & " " & VSuper_Nm & " " & Format(Now, "HH:mm:ss"))
                        vgtotal.Text = CStr(CDbl(vgtotal.Text) - Val(.Item(10, DataGridView1.CurrentRow.Index).Value))
                        lblqty.Text = CStr(Val(lblqty.Text) - Val(.Item(5, DataGridView1.CurrentRow.Index).Value))
                        lblgrand_total.Text = CDec(vgtotal.Text).ToString("N0")
            ...
            Case 5 'Validate detail
                If cmdsales(6).Text = "ESC" Then Exit Sub
                With DataGridView1
                    Try
                        aa = .Item(2, DataGridView1.CurrentRow.Index).Value & " " & CDec(.Item(5, DataGridView1.CurrentRow.Index).Value).ToString("N0") & " x " & CDec(.Item(4, DataGridView1.CurrentRow.Index).Value).ToString("N0")
                        If CDbl(.Item(6, DataGridView1.CurrentRow.Index).Value) > 0 Then aa = aa & vbNewLine & "Disc." & .Item(6, DataGridView1.CurrentRow.Index).Value & "% = " & CDec(.Item(7, DataGridView1.CurrentRow.Index).Value).ToString("N0")
                        If CDbl(.Item(8, DataGridView1.CurrentRow.Index).Value) > 0 Then aa = aa & vbNewLine & "Disc." & .Item(8, DataGridView1.CurrentRow.Index).Value & "% = " & CDec(.Item(9, DataGridView1.CurrentRow.Index).Value).ToString("N0")
                        ...
                        bb = .Item(3, DataGridView1.CurrentRow.Index).Value & " Rp. " & CDec(.Item(10, DataGridView1.CurrentRow.Index).Value).ToString("N0") & vbNewLine & .Item(1, DataGridView1.CurrentRow.Index).Value & "/" & VKasir_Nm
                        If Trim(aa) <> "X" Then Call CetakValid(vno_trans.Text, aa, bb)
                        ...
                        'Tanpa Kasir
                        Call SaveLog(Me.Name & " " & "Validasi Kasir : " & VKasir_ID & "/" & VKasir_Nm & "SPG : " & .Item(1, DataGridView1.CurrentRow.Index).Value & "/" & VKasir_Nm & " " & Format(Now, "HH:mm:ss"))
            ...
            Case 6 'Esc atau validate total
                If cmdsales(6).Text = "SHOPPING BAG" Then
                    'Call Kosong()
                    Me.TopMost = False
                    frmSizeBagAll.ShowDialog()
                    frmSizeBagAll.TopMost = True
                    txtkode.Focus()
                    If txtkode.Text <> "" Then System.Windows.Forms.SendKeys.Send("{Enter}")
                Else
                    frmDisc.lblmsg.Text = "VALIDASI"
                    frmDisc.ShowDialog()
                End If
                txtkode.Focus()
            Case 7 'Close
                ...
                If dsSales.Tables(0).Rows.Count > 0 And Label1.Text = "TOTAL : Rp" Then Call CetakPesan("HOLD", vno_trans.Text)
                If dsSales.Tables(0).Rows.Count = 0 Then getSqldb("delete from sales_transactions where transaction_number = '" & vno_trans.Text & "'", ConnLocal)
                        ...
                        Else
                            If Cari_PLU(VB.Right(Trim(RealPLU), 14)) = True Then
                                ...
                                If CDbl(vflag.Text) = 1 Then
                                    ...
                                    frmHarga.ShowDialog()
                                Else
                                    If Me.Text = "REFUND" Then frmHarga.ShowDialog()
                                End If
                                ...
                                If VOK = True Or CDbl(vflag.Text) = 0 Then
                                    ...
                                    Call Simpan_Header()
                                    Call Simpan_Detail()
                                    Call CDisplay(VB.Left(vdesc.Text, 20), VB.Left(CStr(vqty.Text) & " pcs Rp." & CDec(vharga.Text).ToString("N0"), 20))
                                    'PEMBAYARAN DENGAN POINT REWARDS
                                    'no1
                                    'If Star_No <> "CM000-00000" Then
                                    '    If Linked() Then Call Cari_Item_Rewards((txtkode.Text))
...
loncat:
    ...
    Private Function Cari_SPG(ByRef kode As Object) As Boolean
        ...
        RsCari = getSqldb("select spg_id, spg_name from spg where (spg_id = '" & Trim(kode) & "') or (spg_barcode = '" & Trim(kode) & "')", ConnLocal)
    ...
    Public Sub Simpan_Detail()
        ...
        getSqldb("INSERT INTO Sales_Transaction_Details(Transaction_Number, Seq, PLU, Item_Description, Price, Qty, Amount, " &
                 ...
                 "VALUES ('" & vno_trans.Text & "','" & Gen_Seq(vno_trans.Text) & "','" & vplu.Text & "','" & UbahChar(vdesc.Text) & "','" & vharga.Text & "','" & vqty.Text & "','" & CDbl(vqty.Text) * CDbl(vharga.Text) & "','" & vdisc1.Text & "','" & vdiscrp1.Text & "','" & vdisc2.Text & "','" & vdiscrp2.Text & "','" & vtotal.Text & "','0','0','0','" & VKasir_ID & "','" & vrfid.Text & "')", ConnLocal)
...
ErrH:
    ...
    Public Sub Simpan_DetailRFIDZebra()
        ...
        If dsR.Tables(0).Rows.Count = 0 Then
            getSqldb("INSERT INTO Sales_Transaction_Details(Transaction_Number, Seq, PLU, Item_Description, Price, Qty, Amount, " &
                            "Discount_Percentage, Discount_Amount, ExtraDisc_Pct, ExtraDisc_Amt, Net_Price, Points_Received, Flag_Void, Flag_Status, Flag_Paket_Discount,Epc_Code) " &
                            "VALUES ('" & vno_trans.Text & "','" & Gen_Seq(vno_trans.Text) & "','" & vplu.Text.Trim & "','" & UbahChar(vdesc.Text) & "','" & vharga.Text & "','" & vqty.Text & "','" & CDbl(vqty.Text) * CDbl(vharga.Text) & "','" & vdisc1.Text & "','" & vdiscrp1.Text & "','" & vdisc2.Text & "','" & vdiscrp2.Text & "','" & vtotal.Text & "','0','0','0','" & VKasir_ID & "','" & vrfid.Text.Trim & "')", ConnLocal)
...
ErrH:
        ...
        For Each ro As DataRow In RsPromo.Tables(0).Rows
            ...
            Select Case ro("tipe")
                ...
                Case 6, 7
                    ...
                    RsHitung = Nothing
                Case 17
                    RsHitung = getSqldb("select SUM(qty) as berapa from Sales_Transaction_Details sd inner join PROMO_DTL pd on sd.PLU = pd.PLU " &
                          "where Transaction_Number='" & No_trans & "' and promo_id='" & ro("promo_id") & "'", ConnLocal)
                    If RsHitung.Tables(0).Rows(0).Item("Berapa") >= 2 Then
                        Promo_Disc = ro("disc")
                        If RsHitung.Tables(0).Rows(0).Item("Berapa") >= 3 Then Promo_Disc = ro("txt1")
                    ElseIf RsHitung.Tables(0).Rows(0).Item("Berapa") <= -2 Then
                        Promo_Disc = ro("disc")
                        If RsHitung.Tables(0).Rows(0).Item("Berapa") <= -3 Then Promo_Disc = ro("txt1")
                    Else
                        Promo_Disc = 0
                ...
                Case 18
                    RsHitung = getSqldb("select SUM(qty) as berapa from Sales_Transaction_Details sd inner join PROMO_DTL pd on sd.PLU = pd.PLU " &
                          "where Transaction_Number='" & No_trans & "' and promo_id='" & ro("promo_id") & "'", ConnLocal)
                    If RsHitung.Tables(0).Rows(0).Item("Berapa") >= 1 Then
                        Promo_Disc = ro("disc")
                        If RsHitung.Tables(0).Rows(0).Item("Berapa") >= 2 Then Promo_Disc = ro("txt1")
                    ElseIf RsHitung.Tables(0).Rows(0).Item("Berapa") <= -1 Then
                        Promo_Disc = ro("disc")
                        If RsHitung.Tables(0).Rows(0).Item("Berapa") <= -2 Then Promo_Disc = ro("txt1")
                    Else
                        Promo_Disc = 0
                    ...
                    RsHitung = Nothing
                Case 20
                    RsHitung = getSqldb("select SUM(qty*price) as berapa from Sales_Transaction_Details sd inner join PROMO_DTL pd on sd.PLU = pd.PLU " &
                          "where Transaction_Number='" & No_trans & "' and promo_id='" & ro("promo_id") & "'", ConnLocal)
                    ...
                    If RsHitung.Tables(0).Rows(0).Item("Berapa") > 0 Then 'sales
                        ...
                        jml = Microsoft.VisualBasic.Right(ro("txt1"), 1) * roundDown(RsHitung2.Tables(0).Rows.Count / (Int(Microsoft.VisualBasic.Right(ro("txt1"), 1)) + Int(Microsoft.VisualBasic.Left(ro("txt1"), 1))))


                        StrSQL = "select * from(select top " & jml & " sd.seq, sd.plu from  Sales_Transaction_Details sd inner join PROMO_DTL pd " &
                                 "on sd.PLU = pd.PLU where Transaction_Number='" & No_trans & "' and flag_void=0 and qty=1 and promo_id='" & ro("promo_id") & "' order by amount, net_price ) aa " &
                    ...
                    End If
                    RsHitung.Clear()
                    RsHitung = getSqldb(StrSQL, ConnLocal)

                    Promo_Tipe = ro("Tipe")
                    Promo_Nm = ro("promo_name")

                    If RsHitung.Tables(0).Rows.Count Then
                        Promo_Disc = ro("Disc")
                    Else
                        Promo_Disc = 0
                    End If
    ...
    Private Function Cari_PLU_RFID(ByRef kode As Object, ByRef kode2 As Object) As Boolean
        Dim RsCari As New DataSet
        Cari_PLU_RFID = False
        vpromo.Text = ""
        If Me.Text = "SALES" Then
        ...
        If RsCari.Tables(0).Rows.Count > 0 Then
            If RsCari.Tables(0).Rows(0).Item("TID").ToString.Trim = "" Then
...
1:
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles RfidScan1.Click
        vflag.Text = 0
        vqty.Text = 1
        If Me.Text = "REFUND" Then vqty.Text = CStr(CDbl(vqty.Text) * -1)
        If CDbl(vflag.Text) = 0 And CDbl(vdisc1.Text) <> 0 Then
                ...
                If tagData(nIndex).OpCode = ACCESS_OPERATION_CODE.ACCESS_OPERATION_NONE OrElse (tagData(nIndex).OpCode = ACCESS_OPERATION_CODE.ACCESS_OPERATION_READ AndAlso tagData(nIndex).OpStatus = ACCESS_OPERATION_STATUS.ACCESS_SUCCESS) Then
                    ...
                    Else
                        ...
                        If tag.MemoryBankData.Length > 0 Then
                            If Cari_PLU_RFID(Trim(tag.TagID), Trim(tag.MemoryBankData)) = True Then
                                vqtyRFID = 1
